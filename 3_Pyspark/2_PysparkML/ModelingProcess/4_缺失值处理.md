...
# num col
numCol1 = [i for i in df.columns if re.search('\d+m|\d*days', i)] + ['score']
numCol2 = ['debt_st',
           'is_mi_fan',
           'is_htc_fan',
           'is_4g',
...
# 类别转换及缺失值填补
numColName = numCol1 + numCol2
_numCol1 = [col(i).astype(FloatType()).name('_'+i) for i in numCol1]
_numCol2 = [col(i).astype(FloatType()).name('_'+i) for i in numCol2]
numColName2 = ['_'+i for i in numCol2]

df = df.drop(*numColName).select('*', *[_numCol1+_numCol2]).fillna(-999, numColName2).fillna('_NA').fillna(0)
